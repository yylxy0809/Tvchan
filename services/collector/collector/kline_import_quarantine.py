    ...
    *,
    ...
    for attempt in range(retry_attempts):
        try:
            ...
            if not _is_retryable_transaction_error(exc) or attempt == retry_attempts - 1:
                raise
            # Retry the entire transaction.  This preserves the atomic contract:
            # an aborted attempt cannot leave accepted rows, quarantine evidence,
            # or a durable checkpoint behind.
            await asyncio.sleep(retry_base_delay_seconds * (2 ** attempt))
    raise AssertionError("unreachable")


def _is_retryable_transaction_error(exc: BaseException) -> bool:
    """Recognise PostgreSQL deadlocks and serialization failures without a hard asyncpg import."""
    return getattr(exc, "sqlstate", None) in {"40P01", "40001"}
