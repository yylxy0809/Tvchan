...
本次先盘点和解释，未执行 `git reset`、`git checkout`、`git clean` 或用户文件回滚。下列是提交前的历史工作树统计，相关后端内容现已保存到提交 `d2c4065`：
...
这些数字用于说明改造规模，不再代表当前未提交状态。设备 B 应以共享仓库最新 `master` 为准，而不是按此数字逐个复制。
...
- 后端提交已与 A 的未提交前端改动隔离，但尚未经过 B 新数据库端到端验收。
- 独立服务删除与替代文件新增现已形成同一提交。
...
2. `strategy-service`: 策略代码、合同、测试；大型历史 outputs 不应默认入库。
3. `frontend-windowed-chart`: 由设备 A 单独处理 `apps/web`。
