...
> **状态更新（2026-07-17）：** 本文记录的 foundation -> execution -> replay/gate 历史重建顺序已经通过 PR #9、#10、#11 完成，随后 PR #12、#13 及合入后验收也已完成。不得按本文第 2 至第 4 节重跑历史重建。最新状态与后续优先级以 [14-设备 B 全面接管当前状态](14-device-b-takeover-current-status-2026-07-17.md) 为准。
...
截至 2026-07-16 Gate FE 与接管指令合入后：

| 对象 | 状态 |
| --- | --- |
| `origin/master` | 已包含 PR #5 与 PR #7；PR #7 合入时快照为 `0263508`，实际执行必须重新 fetch 并记录最新 SHA |
| PR #5 | 已于 2026-07-16 合入；Gate FE 证据见 PR #5 comment `issuecomment-4989496334` |
| PR #7 | 已于 2026-07-16 合入；修正分层 rebase 指令并解除 Gate FE |
...
| --- | --- | --- |
| Gate FE | 完成 | 不再等待设备 A |
| Stage B1 foundation rebuild | 待设备 B 执行 | 从旧 foundation tip 重建独立分支和 PR |
| Stage B2 execution rebuild | 阻塞 | 等待 Stage B1 合入 |
| Stage B3 replay/gate rebuild | 阻塞 | 等待 Stage B2 合入 |
| B API canary 与 A 前端验收 | 阻塞 | 等待三层 rebased PR 全部合入 |
...
设备 A 已在 `codex/iwencai-sidebar-v2-rebased` 完成 PR #5 的测试、页面烟测与性能证据，并将其合入 `master`。设备 B 的唯一基线是执行前 fetch 得到的最新 `origin/master`，不得继续使用 PR #5 合入前后的固定 SHA 作为长期基线。

设备 B 现在执行 foundation 重建；在 foundation 新 PR 合入前：

- fetch 并记录最新 `origin/master` SHA，以该快照为新基线，只搬运旧 foundation 的 7 个提交；
...
Write-Host "foundation base=$BASE"
...
git rebase --onto $BASE $OLD_MASTER

# 必须恰好得到 7 个 foundation 增量提交，并证明补丁等价。
git rev-list --count "${BASE}..HEAD"
git range-diff "${OLD_MASTER}..${FOUNDATION}" "${BASE}..HEAD"
...
git rebase --onto $BASE $FOUNDATION
...
git rebase --onto $BASE $EXECUTION
```
...
6. 在 PR 中附上 base SHA、旧/新提交数、`range-diff`、冲突清单和完整测试证据。
...
```text
1. git fetch --prune，记录 origin/master 和 PR #5/#7/#3/#4/#6 的 head/base。
...
Gate FE 已完成。在全面接管完成前，设备 B 的责任是保持已有 replay 证据，严格按 foundation、execution、replay/gate 的顺序重建主干；不是开启另一轮无边界的全量开发或重算。
